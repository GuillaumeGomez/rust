...
pub(crate) fn run(
    ...
    let (CreateRunnableDocTests {
        ...
        ..
    }, source_files) = interface::run_compiler(config, |compiler| {
            ...
            let collector = queries.global_ctxt()?.enter(|tcx| {
                ...
                for file in tcx.sess.source_map().files().iter() {
                    if file.cnum == LOCAL_CRATE {
                        RelativeBytePos.insert(file.name, (file.start_pos, file.source_len.0));
                    }
                }
    ...
    run_unit_tests(opts, &rustdoc_options, &unused_extern_reports, unit_tests, source_files)?;
...
pub(crate) fn run_unit_tests(
    ...
    source_files: FxHashMap<FileName, (BytePos, u32)>,
) -> Result<(), ErrorGuaranteed> {
    ...
    for unit_test in &unit_tests {
        if !unit_test.1.span.is_dummy() {
            file_map.entry(&unit_test.1.filename).or_default().push(unit_test);
        }
    }
    // TODO: Now that we have the list of the source files, generate the doctests based on their
    // span at the beginning (need to update the span to take the block beginning instead of just
    // taking the whole parent) with an attribute `#[doctest]`.
    //
    // Add support for `doctest` attribute in rustc. Likely need a new `--doctest` command line
    // flag.
    //
    // Then generate source files in a folder with the following hierarchy:
    // `[crate_name]/[file path]`.
    //
    // Then you can run `rustc --test` and enjoy everything working smoothly. For the test name,
    // might be a bit tricky, we'll see.
