...
fn assoc_method(
    ...
    let constness = match render_mode {
        RenderMode::Normal => {
            let meth_constness = if tcx.is_const_fn_raw(did) { hir::Constness::Const } else { hir::Constness::NotConst };
            print_constness_with_space(&meth_constness, meth.const_stability(tcx))
        }
        RenderMode::ForDeref { .. } => "",
    };
